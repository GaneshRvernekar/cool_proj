...
const typingTest = document.querySelector(".typing-text");
inputFeild = document.querySelector(".wrapper .input-feild");
...
function randomPara(){
    ...
    // inorder to obtain the actual integer value of a number we use the floar key word
...
// when ever we write into any of the buttons event listener is been triggered over there 

    document.addEventListener("keydown",()=>inputFeild.focus());
}

function initTyping(){
    // if we need the particular starting span of the character then query selector is enough otherwise we need to go with querySelectorAll inorder to obtain all the attributes regardings
    const characters = typingTest.querySelectorAll("span");
    let typed_char= inputFeild.value.split("")[charIndex];
    ...
    if(!isTyping)
    {
        // the problem that was actually occuring was after every click this function was being called which actually should not have occured .
    ...
    if(typed_char==null)
    {
        ...
        // which ever element among the correcr or incorrect present then need to replace the value 
        characters[charIndex].classList.remove("correct","Incorrect");

    }else{
    if(characters[charIndex].innerText === typed_char)
    {
        // if the value found to be correct add the "correct" as also one of its class values .
        // .classlist.add seems very important as it assumes to be adding the classes dynamically to the class
